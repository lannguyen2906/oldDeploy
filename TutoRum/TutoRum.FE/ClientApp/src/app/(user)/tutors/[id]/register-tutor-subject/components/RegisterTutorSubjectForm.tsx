...
const RegisterTutorSubjectForm = ({
  id,
  ...
  const { mutateAsync: register, isLoading } = useRegisterTutorSubject(id);
  ...
  async function onSubmit(values: RegisterLearnerDTOType) {
    try {
      ...
      if (res.status == 201) {
        ...
        route.push("/user/registered-tutors");
      }
    } catch (err) {
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="w-full xl:w-1/2">
                    ...
                    <Select
                      options={tutor?.tutorSubjects
                        ?.filter((s) => s.isVerified == true)
                        .map((subject) => ({
...
export default RegisterTutorSubjectForm;
